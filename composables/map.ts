...
class Map {
	...
	drawPlayer(x: number, y: number, radius: number, player: Player, direction?: number, agent?: boolean, victim?: boolean) {
		...
		if (direction) {
			...
			const triangle_x = canvas_x + Math.cos(radians) * (radius + 2);
			const triangle_y = canvas_y + Math.sin(radians) * (radius + 2);

			this.drawTriangle(triangle_x, triangle_y, 6, radians, player);
		}
		
		if (agent) {
			this.drawAgent(canvas_x, canvas_y, radius * 2, player, victim);
		} else {
	...
	drawTriangle(x: number, y: number, radius: number, radians: number, player: Player) {
		...
		this.ctx.moveTo(x + Math.cos(radians) * radius, y + Math.sin(radians) * radius);
		this.ctx.lineTo(x + Math.cos(radians + (2 * Math.PI) / 3) * radius, y + Math.sin(radians + (2 * Math.PI) / 3) * radius);
	...
	drawAgent (x: number, y: number, radius: number, player: Player, victim?: boolean) {
		...
		this.ctx.drawImage(this.imageCache[agent], x - radius / 2, y - radius / 2, radius, radius);

		// Draw a circle around the agent to show their team
		this.drawPlayerCircle(x, y, radius / 2, player, victim);
		this.ctx.restore();
	}
